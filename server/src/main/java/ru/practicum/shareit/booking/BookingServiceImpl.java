    ...
    public List<BookingDto> readAllByOwnerId(Long sharerId, BookingsState state) {
        ...
        return switch (state) {
            case ALL -> bookings.findAllOwnerItemsBookings(sharerId).stream()
                    .map(BookingMapper::mapToBookingDto)
                    .toList();
            case PAST -> bookings.findAllOwnerItemsBookingsInPast(sharerId).stream()
                    .map(BookingMapper::mapToBookingDto)
                    .toList();
            case FUTURE -> bookings.findAllOwnerItemsBookingsInFuture(sharerId).stream()
                    .map(BookingMapper::mapToBookingDto)
                    .toList();
            case CURRENT -> bookings.findAllCurrentOwnerItemsBooking(sharerId).stream()
                    .map(BookingMapper::mapToBookingDto)
                    .toList();
