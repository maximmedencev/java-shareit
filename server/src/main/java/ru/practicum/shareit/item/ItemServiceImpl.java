...
public class ItemServiceImpl implements ItemService {
    ...
    @Transactional
    @Override
    public ItemDto create(long sharerId, ItemDtoParam itemDtoParam) {
        Item item = ItemMapper.itemDtoParamToItem(itemDtoParam);
        ...
        if (itemDtoParam.getRequestId() != null) {
            item.setRequest(requests.findById(itemDtoParam.getRequestId())
                    .orElseThrow(() -> new NotFoundException("Запрос вещи с id = "
                            + itemDtoParam.getRequestId() + " не найден")));
        }
    ...
    public Collection<ItemDto> readAll(long sharerId) {
        ...
        Collection<ItemDto> sharerItems = items.findByOwnerId(sharerId).stream()
                ...
                .peek(itemDto -> itemDto.setLastBooking(bookings.findLastBookingEndDate(itemDto.getId())))
                .peek(itemDto -> itemDto.setNextBooking(bookings.findNextBookingStartDate(itemDto.getId())))
                .toList();
    ...
    public CommentDto createComment(long sharerId, long itemId, CommentParamDto commentParamDto) {
        ...
        if (!bookings.isUserBookedTheItem(itemId, sharerId)) {
            log.error("Пользователь с id = {} не бронировал вещь с id = {}", sharerId, itemId);
